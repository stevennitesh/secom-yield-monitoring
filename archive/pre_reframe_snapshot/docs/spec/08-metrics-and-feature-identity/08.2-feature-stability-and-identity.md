...
1. `reports/feature_stability_by_seed.csv` is the canonical row-based output for transformed-feature identity plus tuple-level selection indicators across `(selector, seed, outer_fold, feature_index)`.
2. `reports/feature_report.csv` is the canonical report-facing output for frozen-primary feature stability, effect magnitude, expected contribution, and clustering summaries.
3. `reports/drift_gate_summary.csv` consumes the PSI feature-set definition for the frozen models, but its drift-decision contract is governed by Section 07.
