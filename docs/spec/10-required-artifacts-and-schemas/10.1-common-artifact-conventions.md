...
17. `reports/manager_facing_outputs.csv`
18. `reports/feature_report.csv`
19. `reports/drift_gate_summary.csv`
20. `reports/run_manifest.json`
...
2. Lane B artifacts (items 7-19) are not required in this case.
3. The canonical infeasibility signal is manifest-level only: `reports/run_manifest.json` must record `lane_b_feasible=false` and `lane_b_infeasible_reason`.
4. If optional Lane B artifacts are emitted despite infeasibility, they are diagnostic-only and non-canonical for audit and claim workflows.
5. This section does not pin a dedicated in-file `LANE_B_INFEASIBLE` marker schema for those optional diagnostic outputs.
