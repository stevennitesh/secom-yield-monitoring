---
spec_id: "10.4"
title: "Freeze, Lockbox, and MSPC Artifacts"
status: "canonical"
legacy_source:
  - "docs/final_end_to_end_report_strategy_merged.md Section 10"
applies_to:
  - "Lane B"
  - "Phase 2"
  - "Phase 3"
keywords:
  - phase 2
  - lockbox
  - mspc
  - operational cost
---
# 10.4 Freeze, Lockbox, and MSPC Artifacts

## `reports/hyperparameter_freeze_results.csv`

1. One row per `(role, selector, k, C, scaler, n_neighbors, seed, inner_fold)` where `n_neighbors` is nullable for non-ReliefF selectors.
2. Columns: `role`, `selector`, `k`, `C`, `scaler`, `n_neighbors` (nullable for non-ReliefF selectors), `seed`, `inner_fold` where `role in {'primary','challenger'}`.
3. Columns: `inner_ROC_AUC`, `inner_BER`, `mean_inner_ROC_AUC_by_config`, `mean_inner_BER_by_config`, `is_frozen_config`.
4. `mean_inner_*_by_config` are the unweighted arithmetic mean over the 25 `(seed, inner_fold)` observations for that unique `(role, selector, k, C, scaler, n_neighbors)`.
5. Uniqueness constraint: for each `role`, exactly one unique `(selector, k, C, scaler, n_neighbors)` has `is_frozen_config=True` across all `seed x inner_fold` rows.

## `reports/final_lockbox_result.csv`

1. One row per available `(role, threshold_policy)` pair.
2. Columns: `role`, `selector`, `threshold_policy`, `threshold_value`, `BER`, `True+`, `True-`, `ROC_AUC`, `PR_AUC`, `MCC`, `F2`, `lockbox_n`, `lockbox_fails`,
   plus reporting-only matched-point columns: `threshold_at_TNR90`, `TNR_at_TNR90`, `TPR_at_TNR90` (computed on the lockbox slice using the [06 Threshold Policy](../06-threshold-policy.md) `TNR=90%` extraction rule).
3. Allowed `threshold_policy` values are fixed: `scientific` and `operational`.
4. `threshold_at_TNR90`, `TNR_at_TNR90`, and `TPR_at_TNR90` are properties of the model on the lockbox slice and must be identical for the `scientific` and `operational` rows of the same `role`.
5. This artifact is the canonical row-based output of the Lane B frozen-threshold policy:
   1. `threshold_value` records the frozen scientific or operational threshold,
   2. `threshold_at_TNR90` is reporting-only and is not a frozen threshold.

## `reports/mspc_baseline.csv`

1. Columns: `fold_index`, `eval_scope`, `T2_AUC`, `Q_AUC`, `alarm_rate`, `empirical_ARL0`.
2. Columns: `T2_TPR_at_TNR90`, `Q_TPR_at_TNR90`, `best_MSPC_TPR_at_TNR90`, `best_MSPC_source`.
3. Union alarm rule for `alarm_rate` and `empirical_ARL0`:
   `alarm = (T2 > UCL_T2) OR (Q > UCL_Q)` and `alarm_rate = mean(alarm)` over the evaluation slice.
4. `best_MSPC_TPR_at_TNR90 = max(T2_TPR_at_TNR90, Q_TPR_at_TNR90)` and `best_MSPC_source in {'T2','Q'}`.
   Tie-break: if `T2_TPR_at_TNR90 == Q_TPR_at_TNR90`, set `best_MSPC_source='T2'`.
5. `empirical_ARL0` may be `NaN` only when fewer than two alarms occur in the evaluated sequence; if `NaN`, record reason in `reports/run_manifest.json`.
6. Allowed `eval_scope` values are fixed: `outer_fold` and `lockbox`.
7. `fold_index` type is fixed: always write as a string.
   Outer-fold rows serialize the 1-based `outer_fold` as `"1"`, `"2"`, etc; the lockbox row writes `"LOCKBOX"`.
8. Outer-fold rows use `eval_scope='outer_fold'` and `fold_index` equal to the serialized 1-based `outer_fold`.
9. Lockbox row uses `eval_scope='lockbox'` and `fold_index='LOCKBOX'`.

## `reports/operational_cost_curves.csv`

1. One row per `cost_ratio` value in the pre-registered set from [06 Threshold Policy](../06-threshold-policy.md).
2. Columns: `cost_ratio`.
3. Expected cost columns for `primary_scientific`, `primary_operational`, `challenger_scientific`, `challenger_operational`.
4. Plus `all_pass_baseline`, `all_flag_baseline`.
5. Expected cost definition (lockbox evaluation slice):
   1. let `C_FP=1` and `C_FN=cost_ratio` (so `cost_ratio = C_FN / C_FP`),
   2. compute lockbox confusion-matrix counts `FP` and `FN` at the corresponding frozen threshold,
   3. write per-wafer expected cost as `(C_FP*FP + C_FN*FN) / lockbox_n` for each cost column.
6. Baseline cost definitions (lockbox evaluation slice):
   1. `all_pass_baseline = (cost_ratio * lockbox_fails) / lockbox_n` (flags none, so `FP=0` and `FN=lockbox_fails`),
   2. `all_flag_baseline = (lockbox_n - lockbox_fails) / lockbox_n` (flags all, so `FP=lockbox_passes` and `FN=0`).

## `reports/manager_facing_outputs.csv`

1. One row per available `(role, threshold_policy)` pair.
2. Columns:
   `role`, `selector`, `threshold_policy`,
   `dev_sample_count`, `dev_week_count`, `weekly_rate`,
   `predicted_flag_fraction`,
   `mean_weekly_flagged_wafers`, `mean_weekly_fail_captures`, `mean_weekly_fail_misses`,
   `stage_b_mean_flagged_fraction`, `lockbox_flagged_fraction`.
3. Allowed `threshold_policy` values are fixed: `scientific` and `operational`.
4. The full-DEV workload fields are computed from the Phase-3 frozen-model predictions at the corresponding frozen threshold:
   1. `predicted_flag_fraction` is the mean predicted fail fraction on full DEV,
   2. `mean_weekly_flagged_wafers`, `mean_weekly_fail_captures`, and `mean_weekly_fail_misses` are the mean per-active-week counts over DEV.
5. `weekly_rate = dev_sample_count / dev_week_count`, where `dev_week_count` counts only weeks present in DEV.
6. `stage_b_mean_flagged_fraction` is the method-level mean of Stage B `flagged_fraction` across `(outer_fold, seed)` tuples for the corresponding selector.
7. `lockbox_flagged_fraction` is the observed predicted fail fraction on the lockbox slice at the same frozen threshold.
8. This artifact is the canonical row-based output for manager-facing weekly workload and fail-capture summaries.
